    ...
    <main class="flex min-h-screen justify-center px-0 py-0 sm:items-center sm:px-5 sm:py-8">
        <section
            ...
            <div class="flex flex-1 flex-col px-6 py-7 sm:px-7">
                ...
                <a
                    href="{{ route('auth.google.redirect', ['candidate_type' => old('candidate_type', $candidateType ?? 'staff')]) }}"
                    class="mt-6 inline-flex h-[3.25rem] w-full items-center justify-center gap-3 rounded-2xl border border-[#cbd6ec] bg-white px-5 text-[0.98rem] font-bold text-[#223872] transition hover:border-[#4A90E2] hover:bg-white/70 focus:outline-none focus:ring-4 focus:ring-[#4A90E2]/15"
                >
                ...
                <div class="mt-5 flex items-center gap-3 text-xs font-bold uppercase tracking-[0.18em] text-[#94a3b8]">
                    <span class="h-px flex-1 bg-[#dce5f8]"></span>
                    atau
                    <span class="h-px flex-1 bg-[#dce5f8]"></span>
                </div>

                <form action="{{ route('user.register.post') }}" method="POST" class="mt-5 flex flex-1 flex-col">
                    @csrf
