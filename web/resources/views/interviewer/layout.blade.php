...
<body class="min-h-screen bg-[#F4F7FF] font-sans text-[#333333] antialiased">
    @php
        $navGroups = [
            [
                'label' => null,
                'items' => [
                    ['label' => 'Dashboard', 'route' => 'interviewer.dashboard'],
                ],
            ],
            [
                'label' => 'Operasional',
                'items' => [
                    ['label' => 'Pendaftaran', 'route' => 'interviewer.registrations'],
                    ['label' => 'Jadwal Wawancara', 'route' => 'interviewer.schedules'],
                    ['label' => 'Profile Matching', 'route' => 'interviewer.profile-matching'],
                ],
            ],
            [
                'label' => 'Pengaturan',
                'items' => [
                    ['label' => 'Kelola Kriteria', 'route' => 'interviewer.criteria'],
                ],
            ],
        ];
    @endphp
    ...
    <div class="min-h-screen lg:flex">
        <aside data-admin-sidebar class="fixed inset-y-0 left-0 z-40 w-72 -translate-x-full border-r border-[#dce5f8] bg-white shadow-2xl shadow-[#223872]/10 transition lg:static lg:translate-x-0 lg:shadow-none">
            <div class="flex h-full flex-col">
                <div class="flex items-center justify-between border-b border-[#dce5f8] px-5 py-5">
                    <a href="{{ route('interviewer.dashboard') }}" class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs font-semibold text-[#929aaa]">Interviewer Panel</p>
                        </div>
                    </a>

                    <button type="button" data-admin-sidebar-close class="rounded-xl p-2 text-[#64748b] transition hover:bg-[#F4F7FF] lg:hidden" aria-label="Tutup menu">
        ...
        <div data-admin-sidebar-backdrop class="fixed inset-0 z-30 hidden bg-gray-900/50 backdrop-blur-sm lg:hidden"></div>

        <div class="flex-1 flex flex-col min-w-0">
            <header class="sticky top-0 z-20 flex h-20 items-center justify-between border-b border-[#dce5f8] bg-white/80 px-5 backdrop-blur-md lg:px-8">
                <div class="flex items-center gap-4">
                    <button type="button" data-admin-sidebar-open class="rounded-xl p-2 text-[#64748b] transition hover:bg-[#F4F7FF] lg:hidden">
                        <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.2" d="M4 6h16M4 12h16M4 18h16" />
                        </svg>
                    </button>
                    <h2 class="text-lg font-black tracking-tight text-[#111827] sm:text-xl">{{ $title ?? 'Dashboard' }}</h2>
                </div>

                <div class="flex items-center gap-4">
                    <div class="flex items-center gap-3">
                        <div class="hidden text-right sm:block">
                            <p class="text-sm font-bold text-[#111827]">{{ auth()->user()->name ?? 'Interviewer' }}</p>
                            <p class="text-[0.7rem] font-bold uppercase tracking-wider text-[#64748b]">{{ auth()->user()->department->name ?? 'Dept' }}</p>
                        </div>
                        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#223872] text-sm font-black text-white shadow-md shadow-[#223872]/20">
